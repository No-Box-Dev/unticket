...
- `narratePrOpened` → `type='pr_narrative'`, `source='pr-opened-narrator'` — the **PRs feed** (first-person "just opened this PR" post, `PR_OPENED_SYSTEM` prompt). Fires on `github:pr:opened`.
...
- Bot scopes: `channels:read`, `groups:read`, `chat:write`, `chat:write.public` (the last so the bot can post to public channels without being invited). Private channels require manual invite of the bot.
