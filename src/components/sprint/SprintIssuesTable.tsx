    ...
    <div className="bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-700 overflow-hidden">
      <div className="px-4 py-3 border-b border-stone-100 dark:border-stone-800">
        <div className="flex items-center gap-3">
          <span className="text-sm font-medium text-stone-700 dark:text-stone-300">Issues</span>
          <div className="flex rounded-md border border-stone-200 dark:border-stone-700 overflow-hidden text-xs">
            <button
              className={cn(
                ...
                view === "open"
                  ...
                  : "bg-white dark:bg-stone-900 text-stone-500 dark:text-stone-400 hover:bg-stone-50 dark:hover:bg-stone-800/50",
              )}
            ...
            <button
              className={cn(
                "px-2.5 py-1 transition-colors border-l border-stone-200 dark:border-stone-700",
                view === "closed"
                  ? "bg-stone-800 dark:bg-stone-200 text-white dark:text-stone-900"
                  : "bg-white dark:bg-stone-900 text-stone-500 dark:text-stone-400 hover:bg-stone-50 dark:hover:bg-stone-800/50",
              )}
      ...
      <div className="max-h-[400px] overflow-y-auto">
        <div className="min-w-0">
          {isLoading ? (
            ...
            <div className="px-3 py-6 text-center text-stone-400 dark:text-stone-500 text-sm">
            ...
            <div className="divide-y divide-stone-50 dark:divide-stone-800">
              ...
              {hidden > 0 && (
                <div className="px-3 py-1.5 text-xs text-stone-400 dark:text-stone-500 text-center">
                  +{hidden} more {view}
                </div>
              )}
            </div>
...
function IssueRow({ issue, closed }: { issue: SprintIssue; closed: boolean }) {
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-3 px-3 py-2 hover:bg-stone-50 dark:hover:bg-stone-800/50 cursor-pointer text-sm",
        closed && "text-stone-400 dark:text-stone-500",
      )}
      onClick={() => window.open(issue.html_url, "_blank")}
    >
      <span className="text-stone-500 dark:text-stone-400 shrink-0 w-12 text-xs">#{issue.number}</span>
      <span className="truncate flex-1 min-w-0">{issue.title}</span>
      <span className="shrink-0 flex items-center gap-1.5 text-xs text-stone-500 dark:text-stone-400">
      ...
      <span className="shrink-0 w-20 text-stone-500 dark:text-stone-400 text-xs truncate text-right">
