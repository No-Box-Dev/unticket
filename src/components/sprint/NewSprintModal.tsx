...
export function NewSprintModal({ currentSprint, features, onConfirm, onClose, isPending, failedCount }: NewSprintModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div
        className="bg-white dark:bg-stone-900 rounded-xl shadow-xl w-full max-w-md mx-4"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100 dark:border-stone-800">
          <div className="flex items-center gap-2">
            <FastForward size={18} className="text-brand" />
            <h2 className="text-lg font-semibold text-stone-800 dark:text-stone-200 font-display">New Sprint</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          {/* Explanation */}
          <div className="rounded-lg bg-stone-50 dark:bg-stone-800/50 border border-stone-200 dark:border-stone-700 px-4 py-3 text-sm text-stone-600 dark:text-stone-400 space-y-1">
            {productionCount > 0 && (
              <p>
          ...
          <div>
            <label className="text-xs text-stone-500 dark:text-stone-400 block mb-1">Sprint Number</label>
            <div className="px-3 py-2 rounded-md border border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-800/50 text-sm text-stone-700 dark:text-stone-300">
              {nextNumber}
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-md border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 text-sm dark:text-stone-100 focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand"
            />
          </div>
          ...
          <div className="flex items-center gap-2 pt-2">
            ...
            <button
              ...
              onClick={onClose}
              disabled={isPending}
              className="px-4 py-2.5 border border-stone-200 dark:border-stone-700 text-sm text-stone-600 dark:text-stone-400 rounded-lg hover:bg-stone-50 dark:hover:bg-stone-800/50 cursor-pointer disabled:opacity-50"
            >
              Cancel
            </button>
          </div>
