...
const card = "bg-white  border border-stone-200  rounded-xl p-5";
...
export function SprintMetrics({ sprint, sprintFeatures, people, allTasks, tasksLoading }: SprintMetricsProps) {
  ...
  return (
    <div className="space-y-4">
      {aiSummary && (
        <div className="bg-gradient-to-r from-stone-800 to-stone-700 rounded-xl p-5 text-white">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard label="FEATURES" value={featureStats.total} subtitle={`${featureStats.completed} complete`} color="text-stone-700  " />
        <StatCard label="OPEN TASKS" value={taskStats.open} subtitle={`of ${taskStats.total} total`} color="text-stone-700  " />
        <StatCard label="OVERDUE" value={taskStats.overdue} subtitle={taskStats.overdue === 0 ? "on schedule" : "past sprint end"} color={taskStats.overdue > 0 ? "text-severity-high" : "text-stone-300  "} />
        <StatCard label="TASKS DONE" value={taskStats.closed} subtitle={`${taskStats.completionPct}%`} color="text-status-production" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className={card}>
          <h3 className="font-semibold text-stone-900 mb-6">Tasks by State</h3>
          {taskStats.total > 0 ? (
            <DonutChart segments={[
        ...
        <div className={card}>
          <h3 className="font-semibold text-stone-900 mb-4">Tasks per Feature</h3>
        ...
        <div className={card}>
          <h3 className="font-semibold text-stone-900 mb-4">Open Tasks by Assignee</h3>
              ...
              {tasksByAssignee.map(([assignee, count]) => {
                ...
                return (
                  <div key={assignee} className="flex items-center gap-2.5">
                    ...
                    <span className="text-sm text-stone-700 w-24 truncate shrink-0">{displayName}</span>
                    <div className="flex-1 h-3 bg-stone-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${widthPct}%`, backgroundColor: color }} />
                    </div>
                    <span className="text-sm text-stone-500 font-medium tabular-nums w-5 text-right shrink-0">{count}</span>
                  </div>
                );
              })}
            </div>
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="flex flex-wrap justify-center gap-3">
        {segments.map((seg) => (
          <div key={seg.label} className="flex items-center gap-1.5 text-xs text-stone-500">
