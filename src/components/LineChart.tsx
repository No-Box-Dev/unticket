  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        {yTicks.map((v) => (
          <g key={`y-${v}`}>
            <line x1={padL} x2={w - padR} y1={toY(v)} y2={toY(v)} className="stroke-stone-200 dark:stroke-stone-700" strokeWidth={1} />
            <text x={padL - 6} y={toY(v) + 4} textAnchor="end" className="fill-stone-400 dark:fill-stone-500 text-[10px]">
              {v}
            </text>
          </g>
        ))}
        ...
        {/* X-axis labels */}
        {xTicks.map((v) => (
          <text key={`x-${v}`} x={toX(v)} y={height - 4} textAnchor="middle" className="fill-stone-400 dark:fill-stone-500 text-[10px]">
            ...
            .map((line) => {
              ...
              return (
                <text
                  ...
                  y={toY(pt.y) - 10}
                  textAnchor="middle"
                  className="fill-stone-700 dark:fill-stone-300 text-[11px] font-semibold"
                >
                  {pt.y}
                </text>
              );
      ...
      <div className="flex items-center gap-4 mt-2 justify-center">
        {lines.map((line) => (
          <div key={line.label} className="flex items-center gap-1.5 text-xs text-stone-500 dark:text-stone-400">
            <div
              className="w-4 h-0.5"
              style={{
