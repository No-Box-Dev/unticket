...
export function SettingsTab() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-700 p-5 space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-stone-400 dark:text-stone-500">@{user?.login}</div>
          </div>
        </div>
        <div className="text-xs text-stone-400 dark:text-stone-500">
          Organisation: <span className="font-medium text-stone-600 dark:text-stone-400">{selectedOrg}</span>
        </div>
        <button
          onClick={logout}
          ...
          {repos?.map((repo) => {
            ...
            return (
              ...
              >
                {repo.name}
                {isDraft && (
                  <span className="ml-1 text-stone-300 dark:text-stone-600">(draft)</span>
                )}
                {!isDraft && repo.language && (
                  <span className="text-stone-300 dark:text-stone-600 ml-1">({repo.language})</span>
                )}
              </button>
            );
          })}
      ...
      {/* Webhooks */}
      <div className="bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-700 p-5 space-y-3">
        <h2 className="text-sm font-semibold text-stone-900 dark:text-stone-100">Webhooks</h2>
        <p className="text-xs text-stone-400 dark:text-stone-500">
        ...
        <div className="space-y-2">
          <div>
            <label className="text-xs font-medium text-stone-500 dark:text-stone-400">Payload URL</label>
            <div className="mt-0.5 flex items-center gap-2">
              <code className="text-xs bg-stone-100 dark:bg-stone-800 px-2 py-1 rounded text-stone-700 dark:text-stone-300 flex-1 select-all">
                {`${window.location.origin}/api/webhook`}
              </code>
            </div>
          </div>
          <div>
            <label className="text-xs font-medium text-stone-500 dark:text-stone-400">Content type</label>
            <div className="mt-0.5">
              <code className="text-xs bg-stone-100 dark:bg-stone-800 px-2 py-1 rounded text-stone-700 dark:text-stone-300 select-all">
                application/json
              </code>
            </div>
          </div>
          <div>
            <label className="text-xs font-medium text-stone-500 dark:text-stone-400">Events</label>
      ...
      {/* About */}
      <div className="bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-700 p-5 space-y-2">
        <h2 className="text-sm font-semibold text-stone-900 dark:text-stone-100">About unticket.ai</h2>
        <p className="text-xs text-stone-400 dark:text-stone-500">
...
function FullResyncSection() {
  ...
  return (
    <div className="bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-700 p-5 space-y-3">
      <h2 className="text-sm font-semibold text-stone-900 dark:text-stone-100">Data Sync</h2>
      <p className="text-xs text-stone-400 dark:text-stone-500">
...
function AgentIntegrationSection({ org, repos }: { org: string; repos: { name: string }[] | undefined }) {
  ...
  return (
    <div className="bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-700 p-5 space-y-4">
      <div>
        <h2 className="text-sm font-semibold text-stone-900 dark:text-stone-100">Agent Rules</h2>
        <p className="text-xs text-stone-400 dark:text-stone-500 mt-1">
          Rules pushed to each repo's <code className="bg-stone-100 dark:bg-stone-800 px-1 rounded">CLAUDE.md</code> so
      ...
      <div className="space-y-1.5">
        {currentRules.map((rule, i) => (
          <div key={i} className="group flex items-start gap-2 bg-stone-50 dark:bg-stone-800/50 rounded-lg px-3 py-2">
            {editingIdx === i ? (
              <>
                <textarea
                  value={editText}
                  onChange={(e) => setEditText(e.target.value)}
                  className="flex-1 text-xs bg-white dark:bg-stone-900 border border-stone-300 dark:border-stone-600 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-brand/30 resize-y min-h-[60px]"
                  rows={3}
                />
                <button onClick={confirmEdit} className="text-brand hover:text-brand/80 cursor-pointer shrink-0 mt-1">
                  <Check size={14} />
                </button>
                <button onClick={() => setEditingIdx(null)} className="text-stone-400 dark:text-stone-500 hover:text-stone-600 dark:hover:text-stone-400 cursor-pointer shrink-0 mt-1">
                  <X size={14} />
                </button>
              </>
            ) : (
              <>
                <p className="flex-1 text-xs text-stone-700 dark:text-stone-300 whitespace-pre-wrap">{rule}</p>
                <button
                  onClick={() => startEdit(i)}
                  className="text-stone-300 dark:text-stone-600 hover:text-stone-500 dark:hover:text-stone-400 cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
                >
                  <Pencil size={12} />
                </button>
                <button
                  onClick={() => deleteRule(i)}
                  className="text-stone-300 dark:text-stone-600 hover:text-red-500 cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
                >
                  <X size={14} />
                </button>
              </>
      ...
      <div className="flex items-start gap-2">
        <textarea
          ...
          className="flex-1 px-3 py-2 rounded-lg border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 text-xs focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand resize-y min-h-[40px]"
          rows={2}
        />
        <button
          onClick={addRule}
      ...
      {currentRules.length > 0 && (
        <div className="space-y-2 border-t border-stone-100 dark:border-stone-800 pt-4">
          ...
          <div className="grid grid-cols-2 gap-1 max-h-40 overflow-y-auto">
            {repoNames.map((name) => (
              <div key={name} className="flex items-center gap-1.5 px-2 py-1 rounded hover:bg-stone-50 dark:hover:bg-stone-800/50 text-xs text-stone-600 dark:text-stone-400">
                <label className="flex items-center gap-1.5 flex-1 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={selected.has(name)}
                    onChange={() => toggleRepo(name)}
                    className="rounded border-stone-300 dark:border-stone-600 text-brand focus:ring-brand/30"
                  />
                  {name}
                </label>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-stone-300 dark:text-stone-600 hover:text-brand shrink-0"
          ...
          {result && (
            <div className="text-xs space-y-2">
              <div className="space-y-1">
                ...
                {result.skipped > 0 && (
                  <p className="text-stone-400 dark:text-stone-500">{result.skipped} already up to date</p>
                )}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {result.updatedRepos.map((name) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 px-2 py-1 rounded bg-stone-50 dark:bg-stone-800/50 text-stone-600 dark:text-stone-400 hover:text-brand hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors"
                    >
                      {name}
