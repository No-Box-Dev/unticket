  ...
  return (
    <div className="bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-700 p-5 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-stone-900 dark:text-stone-100">Teams</h2>
        {!addingTeam && (
          <button
            onClick={() => {
              setAddingTeam(true);
      ...
      {addingTeam && (
        <div className="border border-stone-200 dark:border-stone-700 rounded-lg p-3 space-y-3 bg-stone-50 dark:bg-stone-800/50">
      ...
      <div className="space-y-2">
        {teams.map((team, i) => {
          ...
          return (
            <div key={i} className="border border-stone-200 dark:border-stone-700 rounded-lg overflow-hidden">
              {/* Team Row */}
              <div className="flex items-center gap-3 px-3 py-2.5">
                ...
                />
                <span className="text-sm font-medium text-stone-800 dark:text-stone-200 flex-1">
                  ...
                  <div className="space-y-2">
                    ...
                    {repos.length > 15 && (
                      <div className="relative">
                        <Search className="w-3.5 h-3.5 absolute left-2 top-1/2 -translate-y-1/2 text-stone-400 dark:text-stone-500" />
                        <input
                          value={repoSearch}
                          onChange={(e) => setRepoSearch(e.target.value)}
                          placeholder="Search repos..."
                          className="w-full text-xs border border-stone-200 dark:border-stone-700 rounded-lg pl-7 pr-2 py-1 focus:outline-none focus:border-teal-600 dark:bg-stone-800 dark:text-stone-100"
                        />
                      </div>
                    )}
                      ...
                      {filteredRepos.map((repo) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span>{repo.name}</span>
                            {isOther && (
                              <span className="text-stone-300 dark:text-stone-600 ml-auto">
      ...
      {PRESET_COLORS.map((c) => (
        <button
          ...
          className={`w-5 h-5 rounded-full cursor-pointer ring-offset-1 ${
            value === c ? "ring-2 ring-stone-400 dark:ring-stone-500" : ""
          }`}
