...
export function PeopleManagement({ people, savePeople, orgMembers, settings, saveSettings }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
      ...
      {members.length > 8 && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full text-xs border border-stone-200 rounded-lg pl-7 pr-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-accent/30"
          />
        </div>
      )}
            ...
            >
              ...
              <div className="flex items-center gap-3 px-3 py-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {member.role && <div className="text-xs text-stone-400">{member.role}</div>}
                </div>
                ...
                {member.active && !isEditing && (
                  <button
                    ...
                    className="p-1 text-stone-400 hover:text-stone-600 cursor-pointer shrink-0"
                  >
                ...
                <div className="border-t border-stone-200 bg-stone-50 p-3 space-y-3">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 text-sm border border-stone-200 rounded-lg px-2.5 py-1 focus:outline-none focus:ring-2 focus:ring-accent/30"
                    />
                    <input
                      value={editRole}
      ...
      {members.length === 0 && (
        <p className="text-xs text-stone-400">No organisation members found. Make sure members are added to your GitHub organisation.</p>
      )}
    </div>
  );
}
